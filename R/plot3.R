...
plot3 <- function(x, region=NULL, decorate=FALSE){
  ...
  if("hospitalized" %in% colnames(x)){
    p <- p + geom_col(aes(x=date, y=hospitalized),fill="blue", col="blue") +
      geom_line(aes(x=date, y=a+b*new.hosp), color="red") +
      geom_point(aes(x=date, y=a+b*new.hosp), color="red")
  }
  p <- p +
  ...
  #death plot
  ylim.prim <- c(0, max(x$death, na.rm=TRUE))   
  ylim.sec <- c(0, max(x$new.deaths, na.rm=TRUE))
  b <- diff(ylim.prim)/diff(ylim.sec)-2
  a <- b*(ylim.prim[1] - ylim.sec[1])
  ...
  pd <- pd+scale_x_date(date_labels = "%b %d") +
    scale_y_continuous("Cumulative Deaths", 
                       sec.axis = sec_axis(~ (. - a)/b, name = "Daily  New Deaths"))
  pd <- pd + geom_line(aes(x=date, y=a+b*new.deaths), linetype="dashed") +
  ...
  yd <- 0.85
  pd <- pd + geom_rect(mapping=aes(xmin=min(x$date), xmax=min(x$date)+xw, ymin=(yd-0.075)*max(x$death), ymax=yd*max(x$death)), fill="red")  +
    annotate("text", x=min(x$date)+xw, y=(yd-0.075/2)*max(x$death),hjust=0, label="  Deaths")
